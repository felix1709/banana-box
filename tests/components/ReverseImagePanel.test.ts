...
describe('ReverseImagePanel', () => {
  ...
  it('handles Tauri string error codes from the reverse-image command', async () => {
    vi.mocked(saveImage).mockResolvedValue('images/rejected-string.png')
    vi.mocked(reverseImagePrompt).mockRejectedValue('PROVIDER_HTTP_ERROR')
    const wrapper = mount(ReverseImagePanel)
    const file = new File(['fake'], 'rejected-string.png', { type: 'image/png' })
